...
class NaverWebtoonCrawler:
    ...
    def __init__(self, webtoon_id): #parameter id
    ...
    def crawl_episode(self, episode_num=None):
        ...
        for index, img in enumerate(img_list):
            ...
            # print(img['src'])
            headers = {'Referer' : url_detail} #헤더 Referer추가  referer는 페이지를 요청한 이전페이지가 무엇인지 알려주는속
            response = requests.get(img['src'],headers=headers)

            # 요청 결과 (이미지파일)의 binary데이터를 파일에 쓴다
            img_path = '{}/{}.jpg'.format(dir_path,index)  #parameter folder directory, index

            # with open(img_path,'wb') as f: #자동열닫 parameter는 img_path ,wb(write,binary)
            #     f.write(response.content)

        #과제 html로 이미지를 볼수 있는 폴더 만들기
        html_path = '{}/{}.html'.format(self.webtoon_id,episode_num)

        html_html ='<html>'
        for index,img in enumerate(img_list):
            html_html += '<img src = "./{}/{}.jpg">'.format(episode_num,index)
